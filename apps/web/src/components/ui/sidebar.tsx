    ...
    >
      ...
      >
        <div
          ...
          className="bg-sidebar group-data-[variant=floating]:ring-sidebar-border group-data-[variant=floating]:rounded-none group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 group-data-[variant=inset]:rounded-lg group-data-[variant=inset]:shadow-sm flex size-full flex-col"
        >
          {children}
        </div>
      </div>
...
function SidebarTrigger({
  ...
  onToggle,
  ...props
}: React.ComponentProps<typeof Button> & { onToggle?: () => void }) {
  const context = useSidebarOptional();
  const toggle = onToggle ?? context?.toggleSidebar;
